...
# Build UPX.
FROM --platform=$BUILDPLATFORM alpine:3.19 AS upx
...
FROM --platform=$BUILDPLATFORM alpine:3.19 AS tigervnc
ARG TARGETPLATFORM
COPY --from=xx / /
...
# Build Fontconfig.
FROM --platform=$BUILDPLATFORM alpine:3.19 AS fontconfig
ARG TARGETPLATFORM
COPY --from=xx / /
...
# Build Openbox.
FROM --platform=$BUILDPLATFORM alpine:3.19 AS openbox
ARG TARGETPLATFORM
COPY --from=xx / /
